 ...
 */
export function checkIntraSessionAdaptation(
  log: SimulationLog
): InvariantViolation[] {
  ...
  for (const session of log.sessions) {
    ...
    if (!session.adaptationsApplied || session.adaptationsApplied.length === 0)
      continue;
    ...
    const weightAdaptations = session.adaptationsApplied.filter(
      (a) =>
        a.adaptationType === 'weight_reduced' ||
        a.adaptationType === 'sets_capped'
    );
    ...
    if (weightAdaptations.length > 0) {
      ...
      for (
        let i = 0;
        i < Math.min(originalSets.length, adaptedSets.length);
        i++
      ) {
        ...
        const reductionPct =
          (original.weight_kg - adapted.weight_kg) / original.weight_kg;
