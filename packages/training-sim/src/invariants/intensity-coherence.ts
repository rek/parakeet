...
export function checkIntensityCoherence(
  log: SimulationLog
): InvariantViolation[] {
  ...
  for (const session of log.sessions) {
    ...
    const oneRm = log.oneRmProgression.find(
      (p) => p.weekNumber === session.weekNumber
    );
    if (oneRm) {
  ...
  for (const session of log.sessions) {
    ...
    const regularSessions = log.sessions.filter(
      (s) =>
        !s.skipped &&
        !s.isDeload &&
        s.primaryLift === session.primaryLift &&
        s.mainLiftSets.length > 0
    );
    if (regularSessions.length === 0) continue;

    const avgRegularWeight =
