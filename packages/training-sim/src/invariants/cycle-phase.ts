...
export function checkCyclePhaseCompliance(log: SimulationLog): InvariantViolation[] {
  ...
  for (const session of sessionsWithPhase) {
    ...
    if (phase === 'menstrual' || phase === 'late_luteal') {
      const hasCycleRationale = session.jitOutput.rationale.some(
        (r: string) => r.toLowerCase().includes('cycle') || r.toLowerCase().includes('menstrual') || r.toLowerCase().includes('luteal'),
      )
