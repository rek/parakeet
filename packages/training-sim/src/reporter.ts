...
export function formatReport(report: SimulationReport): string {
  ...
  lines.push(
    `# Simulation Report: ${report.persona.name} × ${report.script.name}`
  );
  lines.push('');
  lines.push(`## Persona`);
  lines.push(
    `- ${report.persona.name}, ${report.persona.ageYears}yo ${report.persona.biologicalSex}, ${report.persona.bodyweightKg}kg`
  );
  lines.push(
    `- S/B/D: ${report.persona.squatMaxKg}/${report.persona.benchMaxKg}/${report.persona.deadliftMaxKg} kg`
  );
  ...
  lines.push(
    `- Total violations: ${report.summary.totalViolations} (${report.summary.errors} errors, ${report.summary.warnings} warnings)`
  );
  ...
  if (report.log.oneRmProgression.length > 1) {
    ...
    const last =
      report.log.oneRmProgression[report.log.oneRmProgression.length - 1];
  ...
  if (report.violations.length > 0) {
    ...
    for (const [category, categoryViolations] of Object.entries(byCategory)) {
      ...
      for (const v of categoryViolations) {
        const loc = v.weekNumber
          ? ` (week ${v.weekNumber}${v.day != null ? `, day ${v.day}` : ''})`
          : '';
...
function buildWeeklyVolumeSummary(
  log: SimulationLog
): Array<{ weekNumber: number; volume: Record<string, number> }> {
