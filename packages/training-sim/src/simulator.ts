...
function getCyclePhaseForDay(dayInCycle: number, cycleLength: number): CyclePhase {
...
export function runSimulation(options: SimulatorOptions): SimulationLog {
  ...
  const baseMrvMev = persona.biologicalSex === 'female'
    ? DEFAULT_MRV_MEV_CONFIG_FEMALE
    : DEFAULT_MRV_MEV_CONFIG_MALE
  ...
  const activeDisruptions: Array<{ disruption: DisruptionEvent; startDay: number; asTrainingDisruption: TrainingDisruption }> = []
  ...
  let periodStartDay: number | null = persona.biologicalSex === 'female' ? 0 : null
  ...
  for (let day = 0; day < totalDays; day++) {
    ...
    const jitInput: JITInput = {
      ...
      warmupConfig: { type: 'preset', name: persona.biologicalSex === 'female' ? 'standard_female' : 'standard' },
    ...
    const simulatedRpe = simulateRpe(
      targetRpe,
      weekInBlock,
      performanceModel,
    )
    ...
    const volumeStatus = classifyVolumeStatus(weeklyVolume as Record<MuscleGroup, number>, mrvMevConfig)
...
function makeSkippedSession(day: number, scaffold: SessionScaffold, reason?: string): SimulatedSession {
  return {
    day,
    weekNumber: scaffold.weekNumber,
    dayNumber: scaffold.dayNumber,
...
function buildCompletedSetLogs(
  primaryLift: Lift,
  jitOutput: { mainLiftSets: Array<{ rpe_target?: number }>; auxiliaryWork: Array<{ exercise: string; sets: unknown[]; skipped: boolean }> },
): CompletedSetLog[] {
...
function toTrainingDisruption(event: DisruptionEvent, day: number): TrainingDisruption {
