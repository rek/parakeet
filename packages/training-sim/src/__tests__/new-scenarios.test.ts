...
import { formatReportJson, generateReport } from '../reporter';
import {
...
describe('No equipment script', () => {
  it('runs equipment unavailable scenario', () => {
    ...
    const duringDisruption = log.sessions.filter(
      (s) =>
        !s.skipped &&
        s.day >= log.disruptions[0].day &&
        s.day < log.disruptions[0].resolvedDay
    );
    expect(duringDisruption.length).toBeGreaterThan(0);

    // Should see bodyweight rationale
    const hasBodyweightRationale = duringDisruption.some((s) =>
      s.jitOutput.rationale.some(
        (r: string) =>
          r.toLowerCase().includes('bodyweight') ||
...
describe('Fatigue accumulation script', () => {
  it('runs fatigue buildup scenario', () => {
    ...
    const hasSorenessAdjustment = week8_9.some((s) =>
      s.jitOutput.rationale.some(
        (r: string) =>
          r.toLowerCase().includes('soreness') ||
          r.toLowerCase().includes('readiness') ||
