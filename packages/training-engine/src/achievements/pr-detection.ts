...
function hasMajorDisruption(
  disruptions: Array<{ severity: Severity }> | undefined
): boolean {
...
export function detectSessionPRs(input: PRCheckInput): PR[] {
  const { sessionId, lift, completedSets, historicalPRs, activeDisruptions } =
    input;
  ...
  const best1rmSet = completedSets
    .filter(
      (s) =>
        s.rpe !== undefined &&
        s.rpe >= RPE_ELIGIBILITY_THRESHOLD &&
        s.estimated1rmKg !== undefined
    )
    .reduce<{ estimated1rmKg: number } | null>((best, s) => {
      const e1rm = s.estimated1rmKg as number;
      if (best === null || e1rm > best.estimated1rmKg)
        return { estimated1rmKg: e1rm };
      return best;
    }, null);

  if (
    best1rmSet !== null &&
    best1rmSet.estimated1rmKg > historicalPRs.best1rmKg
  ) {
    results.push({
      type: 'estimated_1rm',
      lift,
...
export function checkCycleCompletion(
  ...
  const { totalScheduledSessions, completedSessions, skippedWithDisruption } =
    input;
