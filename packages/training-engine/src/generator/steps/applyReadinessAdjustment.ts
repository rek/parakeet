...
export function applyReadinessAdjustment(ctx: PipelineContext, input: JITInput, traceBuilder?: PrescriptionTraceBuilder) {
  ...
  if (readinessModifier.setReduction > 0 || readinessModifier.intensityMultiplier !== 1.0) {
    ctx.plannedCount = Math.max(1, ctx.plannedCount - readinessModifier.setReduction);
    ctx.intensityMultiplier *= readinessModifier.intensityMultiplier;
    if (readinessModifier.rationale) ctx.rationale.push(readinessModifier.rationale);
  }

  ctx.readinessSetsRemoved = preCount - ctx.plannedCount;
  if (ctx.readinessSetsRemoved > 0) {
    traceBuilder?.recordVolumeChange({ source: 'readiness', setsBefore: preCount, setsAfter: ctx.plannedCount, reason: readinessModifier.rationale ?? 'Readiness set reduction' });
  }
}
