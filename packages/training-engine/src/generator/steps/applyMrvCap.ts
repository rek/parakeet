...
export function applyMrvCap(ctx: PipelineContext, input: JITInput, traceBuilder?: PrescriptionTraceBuilder) {
  ...
  for (const { muscle, contribution } of liftMuscles) {
    ...
    if (ctx.plannedCount > remainingSets) {
      const prevCount = ctx.plannedCount;
      ctx.warnings.push(`Approaching MRV for ${muscle} — sets capped at ${remainingSets}`);
