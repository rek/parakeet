...
export function buildFinalMainSets(ctx: PipelineContext, input: JITInput, traceBuilder?: PrescriptionTraceBuilder): PlannedSet[] {
  const barWeightKg = input.barWeightKg ?? 20;

  if (ctx.inRecoveryMode) {
    const recoveryWeight = Math.max(barWeightKg, roundToNearest(ctx.baseWeight * 0.4));
    ...
    traceBuilder?.recordSets(sets.map((s) => ({
