...
export function applyCyclePhaseAdjustment(ctx: PipelineContext, input: JITInput, traceBuilder?: PrescriptionTraceBuilder) {
  ...
  if (cyclePhaseModifier.volumeModifier !== 0 || cyclePhaseModifier.intensityMultiplier !== 1.0) {
    ctx.plannedCount = Math.max(1, ctx.plannedCount + cyclePhaseModifier.volumeModifier);
    ctx.intensityMultiplier *= cyclePhaseModifier.intensityMultiplier;
    if (cyclePhaseModifier.rationale) ctx.rationale.push(cyclePhaseModifier.rationale);
  }

  ctx.cyclePhaseSetsRemoved = preCount - ctx.plannedCount;
  if (ctx.cyclePhaseSetsRemoved > 0) {
    traceBuilder?.recordVolumeChange({ source: 'cycle_phase', setsBefore: preCount, setsAfter: ctx.plannedCount, reason: cyclePhaseModifier.rationale ?? 'Cycle phase set reduction' });
  }
}
