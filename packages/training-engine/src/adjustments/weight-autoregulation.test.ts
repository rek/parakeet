...
import {
  evaluateWeightAutoregulation,
  WeightAutoregulationContext,
} from './weight-autoregulation';

function makeCtx(
  overrides: Partial<WeightAutoregulationContext> = {}
): WeightAutoregulationContext {
  return {
    rpeActual: 6.5,
    rpeTarget: 8.0,
...
describe('evaluateWeightAutoregulation', () => {
  ...
  it('bench, gap >= 1.0 → +2.5 kg', () => {
    const result = evaluateWeightAutoregulation(
      makeCtx({ rpeActual: 7.0, rpeTarget: 8.0 })
    );
  ...
  it('bench, gap >= 1.5 → +5 kg', () => {
    const result = evaluateWeightAutoregulation(
      makeCtx({ rpeActual: 6.5, rpeTarget: 8.0 })
    );
    expect(result).not.toBeNull();
  ...
  it('gap < 1.0 → null', () => {
    const result = evaluateWeightAutoregulation(
      makeCtx({ rpeActual: 7.5, rpeTarget: 8.0 })
    );
  ...
  it('hasAlreadyAdjusted → null', () => {
    const result = evaluateWeightAutoregulation(
      makeCtx({ hasAlreadyAdjusted: true })
    );
    expect(result).toBeNull();
  });
