...
function makeDisruption(
  ...
  return { disruption_type, severity, affected_lifts } as Parameters<
    typeof suggestDisruptionAdjustment
  >[0];
}

const squat: PlannedSession = {
  id: 's1',
  primary_lift: 'squat',
  status: 'planned',
};
const bench: PlannedSession = {
  id: 's2',
  primary_lift: 'bench',
  status: 'planned',
};
const deadlift: PlannedSession = {
  id: 's3',
...
describe('suggestDisruptionAdjustment — injury', () => {
  ...
  it('moderate injury → 40% weight_reduced', () => {
    const result = suggestDisruptionAdjustment(
      makeDisruption('injury', 'moderate'),
      [squat]
    );
    expect(result[0].action).toBe('weight_reduced');
...
describe('suggestDisruptionAdjustment — illness', () => {
  ...
  it('moderate illness → weight_reduced + reps_reduced per session', () => {
    const result = suggestDisruptionAdjustment(
      makeDisruption('illness', 'moderate'),
      [squat]
    );
    expect(result).toHaveLength(2);
    expect(
      result.find((r) => r.action === 'weight_reduced')?.reduction_pct
    ).toBe(25);
    expect(
      result.find((r) => r.action === 'reps_reduced')?.reps_reduction
    ).toBe(2);
  });

  it('minor illness → reps_reduced by 2 only', () => {
    const result = suggestDisruptionAdjustment(
      makeDisruption('illness', 'minor'),
      [squat]
    );
    expect(result).toHaveLength(1);
...
describe('suggestDisruptionAdjustment — equipment_unavailable', () => {
  it('returns exercise_substituted with substitution note', () => {
    const result = suggestDisruptionAdjustment(
      makeDisruption('equipment_unavailable', 'minor'),
      [squat]
    );
