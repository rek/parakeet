...
function makeLog(
  overrides: Partial<SessionLogSummary> &
    Pick<SessionLogSummary, 'lift' | 'intensity_type'>
): SessionLogSummary {
  return {
...
describe('suggestProgramAdjustments — max_suggestions_per_lift', () => {
  it('high RPE on both heavy and explosive → only 1 suggestion (max=1)', () => {
    ...
    expect(
      result.filter(
        (s) => s.type !== 'flag_for_review' && s.affected_lift === 'squat'
      )
    ).toHaveLength(1);
  });
  ...
  it('custom max=2 allows two suggestions for same lift', () => {
    ...
    const result = suggestProgramAdjustments(logs, {
      ...DEFAULT_THRESHOLDS_MALE,
      max_suggestions_per_lift: 2,
    });
    expect(
      result.filter(
        (s) => s.type !== 'flag_for_review' && s.affected_lift === 'squat'
