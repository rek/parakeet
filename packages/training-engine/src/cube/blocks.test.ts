...
describe('DEFAULT_FORMULA_CONFIG_FEMALE', () => {
  ...
  it('bench_max is 2.5kg', () => {
    expect(DEFAULT_FORMULA_CONFIG_FEMALE.training_max_increase.bench_max).toBe(
      2.5
    );
  });

  it('squat_max and deadlift_max are 7.5kg', () => {
    expect(DEFAULT_FORMULA_CONFIG_FEMALE.training_max_increase.squat_max).toBe(
      7.5
    );
    expect(
      DEFAULT_FORMULA_CONFIG_FEMALE.training_max_increase.deadlift_max
    ).toBe(7.5);
  });
