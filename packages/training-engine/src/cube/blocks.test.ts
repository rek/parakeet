...
describe('formula config behavioral properties', () => {
  it('female config produces more sets than male in block 1 heavy', () => {
    const femaleSets = calculateSets(
      'squat',
      'heavy',
      1,
      100,
  ...
  it('female training max increase is more conservative than male for squat', () => {
    expect(
      DEFAULT_FORMULA_CONFIG_FEMALE.training_max_increase.squat_max
    ).toBeLessThan(DEFAULT_FORMULA_CONFIG_MALE.training_max_increase.squat_max);
  });

  it('deload produces lighter sets than any working block', () => {
    const deload = calculateSets(
      'squat',
      'deload',
      1,
      140,
    ...
    const minWorkingWeight = Math.min(
      b1[0].weight_kg,
      b2[0].weight_kg,
      b3[0].weight_kg
    );
...
describe('getDefaultFormulaConfig', () => {
  it('"female" → female config', () => {
    expect(getDefaultFormulaConfig('female')).toBe(
      DEFAULT_FORMULA_CONFIG_FEMALE
    );
  });
  ...
  it('undefined → male config', () => {
    expect(getDefaultFormulaConfig(undefined)).toBe(
      DEFAULT_FORMULA_CONFIG_MALE
    );
  });
});
