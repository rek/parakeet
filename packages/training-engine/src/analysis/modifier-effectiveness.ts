...
export type ModifierSource = 'rpe_history' | 'readiness' | 'cycle_phase' | 'soreness' | 'disruption';
...
export interface ModifierSample {
  ...
  rpeTarget: number;
  rpeActual: number;
  /** For soreness: the soreness level that triggered the modifier (1-5). */
 ...
 */
export function computeCalibrationBias({ samples }: { samples: ModifierSample[] }) {
  if (samples.length === 0) {
    return {
  ...
  const meanBias = samples.reduce((sum, s) => sum + (s.rpeActual - s.rpeTarget), 0) / n;
  ...
  const confidence: CalibrationConfidence =
    n >= CONFIDENCE_THRESHOLDS.high ? 'high' :
    n >= CONFIDENCE_THRESHOLDS.medium ? 'medium' :
    n >= CONFIDENCE_THRESHOLDS.low ? 'low' :
    'exploring';
 ...
 */
export function shouldTriggerReview({ calibration, previousAdjustment }: {
  ...
  if (Math.abs(calibration.suggestedAdjustment) > AUTO_APPLY_THRESHOLD) return true;
  ...
  if (previousAdjustment !== undefined && previousAdjustment !== 0) {
    const flipped = Math.sign(calibration.suggestedAdjustment) !== Math.sign(previousAdjustment);
 ...
 */
export function canAutoApply({ calibration }: { calibration: CalibrationResult }) {
  if (calibration.confidence === 'exploring' || calibration.confidence === 'low') return false;
  if (Math.abs(calibration.suggestedAdjustment) > AUTO_APPLY_THRESHOLD) return false;
  return true;
}
 ...
 */
export function extractModifierSamples({ modifiers, rpeTarget, rpeActual }: {
  modifiers: Array<{ source: ModifierSource; multiplier: number; reason: string }>;
  rpeTarget: number;
  rpeActual: number;
}) {
 ...
 */
export function applyCalibrationAdjustment({ defaultMultiplier, adjustment }: {
