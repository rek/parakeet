...
**Why this matters:** the weight scaling system (`computeAuxWeight`) detects implement type by checking whether the name _starts with_ `Dumbbell` or `Kettlebell`. A name like `Romanian Dumbbell Deadlift` would silently get barbell linear scaling instead of the correct sqrt scaling. Always put the equipment first.
