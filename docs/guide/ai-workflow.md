...
**Refactors need specs too** — structural refactors (consolidating duplicated logic) warrant a brief spec update even if no user-visible behavior changes. Update the existing spec to reflect the new file and call site, and note the refactor in implementation-status.md.

**backlog.md as an agent task queue** — `docs/backlog.md` is the single task queue for AI agents (features and bugs merged). Each item is numbered with user-voice description and links to relevant design/spec docs. The agent reads the list, picks the next item, implements it following the full AI workflow (orient → design → plan → implement → validate → wrap up), and updates docs.
