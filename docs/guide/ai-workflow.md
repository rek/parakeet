...
**Never fire-and-forget AsyncStorage writes in event handlers** — `setDisabledPlates(plates)` without `await` in `handleDisabledPlatesChange` silently lost plate availability settings (GH#91). AsyncStorage writes in component event handlers must be awaited with try/catch + `captureException`. Fire-and-forget promises can be interrupted by screen unmounts or navigation, and errors are swallowed. Pattern: any handler that persists user preferences to AsyncStorage should be `async`, `await` the write, and catch errors. This applies to all settings-style writes, not just plates.
...
**Timer attribution must survive every entry point** — `openTimer()` accepts optional attribution fields (`pendingMainSetNumber`, `pendingAuxExercise`). When a new call site omits them, `handleTimerDone` silently falls through all branches and shows nothing. Every `openTimer()` call must either pass attribution or the downstream handler must have an explicit fallback branch for the unattributed case. Pattern: when adding a new timer trigger (post-warmup, post-exercise-swap, etc.), trace through `handleTimerDone` → `PostRestOverlay` → `handleLiftComplete`/`handleLiftFailed` → `computeDismissResult` and verify the full chain produces a valid result.
