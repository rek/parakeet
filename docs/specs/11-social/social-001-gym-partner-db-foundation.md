...
- [ ] `updated_at` auto-update trigger: `BEFORE UPDATE` sets `updated_at = now()` — repository does not manually set `updated_at`
- [ ] Indexes:
...
- [ ] `token` column DEFAULT `gen_random_uuid()::text` — token generated by Postgres, not client-side (`crypto.randomUUID()` is not available in React Native / Hermes)
...
- [ ] `updatePartnerStatus(id, status): Promise<void>` — accept/decline/remove. Fetches current row to resolve caller's role (requester/responder), validates via `canTransition()` before UPDATE. Throws on invalid transitions.
