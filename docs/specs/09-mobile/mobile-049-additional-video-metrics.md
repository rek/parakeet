...
- [ ] `computeFatigueSignatures({ reps }): FatigueSignatures` — takes the `reps[]` array from `VideoAnalysisResult`, returns per-metric deltas from rep 1 to rep N:
...
- [ ] Add `FatigueSignaturesSchema` to `packages/shared-types/src/video-analysis.schema.ts`
- [ ] Add `fatigueSignatures` field to `VideoAnalysisResultSchema` (optional, top-level)
- [ ] Wire into `assembleAnalysis` after rep loop (takes `repsWithVelocity` array)
- [ ] Tests: flat set (no fatigue), progressive fatigue, improving set, mixed
...
- [ ] `detectButtWink({ frames, bottomFrame, fps }): { detected: boolean; magnitude: number | null; frameIndex: number | null }`
...
- [ ] Add to `RepAnalysis`: `buttWinkDeg: z.number().optional()`
- [ ] Add as a fault: `type: 'butt_wink'`, severity `'warning'`
- [ ] Tests: clean squat (no wink), wink at bottom, gradual hip flexion (not wink)

### 2.2 — Stance Width

**`modules/video-analysis/lib/stance-width.ts`:**

- [ ] `computeStanceWidth({ frame }): number` — `|leftAnkle.x - rightAnkle.x| × CM_PER_UNIT`
  - Best from front view; side view gives compressed projection
  - Compute at standing frame (first or last frame of rep)
- [ ] Add to `RepAnalysis`: `stanceWidthCm: z.number().optional()`
- [ ] Tests: narrow, medium, wide stance

### 2.3 — Hip Shift / Lateral Lean

**`modules/video-analysis/lib/hip-shift.ts`:**

- [ ] `computeHipShift({ frames, startFrame, endFrame }): { maxShiftCm: number; direction: 'left' | 'right' | 'none' }`
  - Track `(leftHip.y - rightHip.y) × CM_PER_UNIT` per frame
  - Return max asymmetry and which side drops
  - Only meaningful from front view
- [ ] Add to `RepAnalysis`: `hipShiftCm: z.number().optional()`, `hipShiftDirection: z.string().optional()`
- [ ] Tests: symmetric, left shift, right shift
...
- [ ] `computeElbowFlare({ frame }): number` — angle between upper arm (shoulder→elbow) and torso (shoulder→hip) at bottom frame
  - Side view: compute on the visible side
  - Ideal range: 45-75° depending on grip width
- [ ] Add to `RepAnalysis`: `elbowFlareDeg: z.number().optional()`
- [ ] Add as fault when >80° or <30°: `type: 'elbow_flare'`
- [ ] Tests: optimal (60°), excessive (85°), overtucked (25°)

### 3.2 — Pause Quality (Sink Detection)

**`modules/video-analysis/lib/pause-quality.ts`:**

- [ ] `assessPauseQuality({ repPath, fps }): { pauseDurationSec: number; isSinking: boolean }`
...
- [ ] Add to `RepAnalysis`: `pauseDurationSec: z.number().optional()`, `isSinking: z.boolean().optional()`
- [ ] Tests: clean pause, sinking, no pause (touch-and-go)
...
- [ ] `analyzeHipHingeTiming({ frames, startFrame, endFrame, fps }): { crossoverPct: number; isEarlyHipShoot: boolean }`
...
- [ ] Add to `RepAnalysis`: `hipHingeCrossoverPct: z.number().optional()`
- [ ] Add as fault when crossoverPct < 30%: `type: 'early_hip_shoot'`
- [ ] Tests: proper timing, early hip shoot, stiff-leg pull

### 4.2 — Bar-to-Shin Distance

**`modules/video-analysis/lib/bar-shin-distance.ts`:**

- [ ] `computeBarToShinDistance({ frames, startFrame, endFrame }): number`
...
- [ ] Add to `RepAnalysis`: `barToShinDistanceCm: z.number().optional()`
- [ ] Add as fault when >5cm: `type: 'bar_away_from_shins'`
- [ ] Tests: bar close, bar drifting forward

## Phase 5 — Lockout Stability (All Lifts)

**`modules/video-analysis/lib/lockout-stability.ts`:**

- [ ] `computeLockoutStability({ frames, endFrame, fps }): number`
...
- [ ] Add to `RepAnalysis`: `lockoutStabilityCv: z.number().optional()`
- [ ] Add as fault when CV > 5%: `type: 'unstable_lockout'`
- [ ] Tests: stable, wobbly, still ascending at end

## Barrel Exports

All new functions should be exported from `modules/video-analysis/index.ts` for use in coaching context and UI display.
