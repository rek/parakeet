...
- [ ] refactor-003: UI-engine decoupling (GH#132) — Phases 0-2, 7 complete: weight utils → `@shared/utils/`; muscle types → `@parakeet/shared-types`; plates/calculatePlates → `@shared/constants/plates`; exercise-lookup → `@shared/utils/exercise-lookup`; `DEFAULT_RPE_TARGET`/`PRIMARY_CONTRIBUTION_THRESHOLD` extracted to `@shared/constants/training`; soreness magic numbers replaced. 90→76 engine import files. Phases 3-6 remaining.
