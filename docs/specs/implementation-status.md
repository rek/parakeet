...
- [ ] refactor-003: UI-engine decoupling (GH#132) — Phases 0-2 complete: weight utils moved to `@shared/utils/`; muscle types promoted to `@parakeet/shared-types`; `PlateKg`/`PLATE_COLORS`/`calculatePlates` moved to `@shared/constants/plates`; exercise-lookup adapter in `@shared/utils/exercise-lookup`. 90→76 engine import files. Phases 3-7 remaining.
