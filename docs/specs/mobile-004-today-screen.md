...
The home tab that shows the user's next upcoming session and any active edge case banners.
...
- Active edge case banner above the card if any active edge cases for today's lift
  - Amber background strip: "Knee injury active — Squat load reduced 20%"
  - Tap → navigate to `edge-case/report.tsx` in detail view
...
- [edge-cases-003-resolution-api.md](./edge-cases-003-resolution-api.md)
