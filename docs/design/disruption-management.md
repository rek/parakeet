...
Disruption Management lets users communicate real-world disruptions — injury, illness, travel, extreme fatigue, equipment unavailability — so Parakeet can respond with appropriate program adjustments rather than letting those deviations silently undermine the training cycle.
...
10. Active disruption appears as a banner on the Today screen until resolved
...
- [ ] Should the system automatically reduce load without user confirmation for minor disruptions, or always require explicit approval?
- [ ] How do we handle a disruption that overlaps with a deload week — no adjustment needed?
