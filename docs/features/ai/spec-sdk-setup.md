...
- [ ] **Expired JWT during a flight call returns 401 with no retry.** Proxy validates JWT via `adminClient.auth.getUser(token)`; expired token returns 401 with the OpenAI-compatible error envelope. The AI SDK's retry logic treats 4xx as non-retryable, so any LLM call whose JWT expires mid-flight (motivational, JIT-LLM, cycle review) just fails. In `packages/training-engine/src/ai/models.ts`, wrap the proxy `fetch` so that on 401 it calls `supabase.auth.refreshSession()` once and retries; OR have the proxy return 503 with a retryable flag specifically for token errors and let the SDK retry.
- [ ] **Localhost dev silently uses direct OpenAI mode without warning.** `platform/supabase/bootstrap.ts:19-29` skips `configureAIProxy` when `supabaseUrl.includes('localhost')`. Dev then pulls the key from `EXPO_PUBLIC_OPENAI_API_KEY` — a bundle-exposed path. If a dev forgets the env var, every LLM call silently 401s. Log a single bootstrap warning when in direct mode with no key present, and document the localhost-proxy port in `guide/dev.md`.
