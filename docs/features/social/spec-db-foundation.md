...
- [x] RLS DELETE: `auth.uid() = inviter_id` (inviter can clean up their own expired invites)
- [x] Auto-cleanup trigger: on INSERT into `gym_partner_invites`, delete rows for same `inviter_id` where `expires_at < now()` (prevents unbounded accumulation of 5-minute TTL invites)
