...
- [x] `claimInvite(token: string): Promise<{ inviterName: string | null }>` — calls `partner.repository.claimInvite(token)` which invokes the `claim_partner_invite(p_token)` SECURITY DEFINER RPC. The RPC atomically claims the invite, inserts the `gym_partners` row with `status: 'pending'` (requester=scanner, responder=inviter), and rolls the claim back on any insert failure so the token stays usable. After a successful claim the client reads the inviter's display_name via a separate `profiles` SELECT (readable post-claim because the new "Read partner profiles" RLS policy permits it).
  - Why an RPC: the previous `.update().select().maybeSingle()` chain returned null `data` even when the row existed and policies should pass. The `.select()` was first paired with a `profiles` embed, then simplified to `.select('inviter_id')` with no embed — both forms still returned null. Most likely cause: PostgREST UPDATE...RETURNING re-evaluates the SELECT RLS policy, and the check sometimes runs against the _pre-update_ row (`claimed_by IS NULL`) rather than the new row, so the `"Inviter and claimer can read invites"` policy filters the row out and PostgREST returns no data. SECURITY DEFINER bypasses RLS for the function body while `auth.uid()` still resolves to the real caller, so `requester_id` cannot be spoofed.
  - Error cases mapped from Postgres error codes: `P0001` already-claimed/invalid, `P0002` self-pairing, `P0003` already paired, `28000` not authenticated. Client switches on `error.code`, not message substring matching.
  - Inviter display_name is intentionally NOT returned: `profiles` RLS only lets the requester read the responder's profile after `status='accepted'`, so the QR success message falls back to "partner" until the inviter accepts. This protects against unsolicited pre-consent disclosure of display_name to anyone holding a target's UUID.
