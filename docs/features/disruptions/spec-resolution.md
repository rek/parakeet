...
- [x] (landed) **Resolving a major disruption does NOT unskip its skipped sessions.** `resolveDisruption` calls `clearSessionJit(sessionIds)`, but `clearSessionJit` only operates on rows with `status = 'planned'`. Sessions that `applyDisruptionAdjustment` marked `'skipped'` stay skipped forever. Real-world impact: travel disruption skips 3 sessions, user comes back early and resolves, those 3 sessions stay dead — user must hand-revive each. Fix: on resolve, restore `status: 'skipped' → 'planned'` for any session whose `planned_date >= today` that this disruption skipped, then `clearSessionJit`.
- [x] (landed) **Open-ended ("ongoing") disruptions never auto-expire.** When a user picks "Ongoing" on the report form, `affected_date_end` is NULL, and `fetchActiveDisruptions` includes any row with `affected_date_end IS NULL` regardless of age. A knee injury logged six months ago that the user forgot about keeps injecting into JIT soreness/disruption inputs indefinitely. Add either (a) a foreground job that surfaces an "Is this still ongoing?" prompt for active disruptions older than N days, or (b) an auto-resolve fallback for injury types older than M days without re-confirmation.
