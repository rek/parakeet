...
- [x] Export `DEFAULT_FORMULA_CONFIG` constant of type `FormulaConfig`
  ...
  - Shape:
    ...
    export const DEFAULT_FORMULA_CONFIG: FormulaConfig = {
      blocks: {
        1: {
          heavy: { pct: 0.8, sets: 2, reps: 5, rpe_target: 8.5 },
          explosive: { pct: 0.65, sets: 3, reps: 8, rpe_target: 7.0 },
          rep: { pct_min: 0.7, pct_max: 0.7, sets_min: 2, sets_max: 3, reps_min: 8, reps_max: 12, rpe_target: 8.0 },
        },
        2: {
          /* ... */
        },
      ...
      deload: { pct: 0.4, sets: 3, reps: 5, rpe_target: 5.0 },
...
- [x] Import and use `DEFAULT_FORMULA_CONFIG` as the base for `getFormulaConfig()`:
  ...
  export async function getFormulaConfig(userId: string): Promise<FormulaConfig> {
    const { data } = await supabase.from('formula_configs').select('*').eq('user_id', userId).eq('is_active', true).maybeSingle();
...
- [engine-003-loading-percentage-calculator.md](../core-engine/spec-loading-calc.md)
