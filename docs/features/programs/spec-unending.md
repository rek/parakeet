...
- [x] **Multi-device race protection — already covered by `sessions_unique_active_slot`.** The existing partial unique index on `(program_id, week_number, day_number) WHERE program_id IS NOT NULL AND status IN ('planned', 'in_progress')` (migration `20260326000000_unique_planned_session_per_slot.sql`) catches concurrent inserts from two devices: both compute the same `(week, day)` from the shared counter and only one INSERT wins. The 23505 retry path in `generateNextUnendingSession` was already designed around this constraint. The in-process JS mutex is a fast-path; the DB constraint is the source of truth. Documented here so a future contributor doesn't add a redundant index on `planned_date`.
- [x] (landed) **`total_weeks` null fall-through in cycle review compilation produces malformed deload analysis.** See `cycle-review/spec-generator.md` open issue — `assemble-cycle-report.ts` treats `0` as a valid `totalWeeks` value and `isDeloadWeek(weekNumber, 0)` returns true everywhere. The `?? 0` shim in `cycle-review.repository.ts:219` was the wrong shape — branch upstream or refuse review generation entirely for unending programs.
- [x] (landed) **Manual "End Program" path for _scheduled_ programs never triggers a cycle review.** `useEndProgram` only passes `triggerCycleReview: isUnending`. A user who hits End on a scheduled program mid-cycle silently archives without a retrospective. Either also fire `onCycleComplete` for scheduled archives ≥ N sessions in, or surface the trade-off in the confirm copy: "No review will be generated unless this program completes 100%."
