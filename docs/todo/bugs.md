# Bugs

This contains a list of currently active bugs. An AI agent should work through this list and resolve them.

When resolved, remove it from here, and update any relevant design/spec docs with changes.

When possible, solve in red/green style.

## 1

plate calculator
i dont understand it, i dont see the plates i need to use
just a bar weight selector

## 2

on the today screen,
resume workout in the from page does nothing
but going to the 'in progress' one allowed me to start that workout again
so something about in progress workouts and being able to resume ones you have started is not working currently

##

when rest timer hits zero, it should buzz or make noise

## 4

cycle tracking:
evaluate the way we track period starting
it seems we can only record 'last period date'
does this keep a record of the previous starts?

## 5

potential bug:
start a workout, but dont finish it
go back to program screen and start a new workout
finish logging that one, something strange is going on
needs further research

