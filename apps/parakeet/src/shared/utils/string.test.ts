...
describe('sessionLabel', () => {
  it('returns "Lift — Intensity" when both are present', () => {
    expect(sessionLabel({ primary_lift: 'squat', intensity_type: 'heavy' })).toBe('Squat — Heavy')
  })

  it('returns just the lift when intensity_type is null', () => {
    expect(sessionLabel({ primary_lift: 'bench', intensity_type: null })).toBe('Bench')
  })

  it('returns activity_name when primary_lift is null', () => {
    expect(sessionLabel({ primary_lift: null, activity_name: 'Morning Mobility' })).toBe('Morning Mobility')
  })

  it('returns "Ad-Hoc Workout" when both primary_lift and activity_name are null', () => {
    expect(sessionLabel({ primary_lift: null, activity_name: null })).toBe('Ad-Hoc Workout')
  })
