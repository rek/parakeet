...
// Helper to build a minimal raw log row for testing
...
describe('processRecentHistory', () => {
  ...
  it('detects improving trend when recent 1RM is substantially higher', () => {
    // older sessions: ~100kg 1RM; recent: ~120kg — delta > 2.5kg
    const rows = [
      // newest (index 0) — higher weight → higher 1RM
  ...
  it('returns stable trend when delta <= 2.5kg', () => {
    // All sessions same weight → delta = 0
