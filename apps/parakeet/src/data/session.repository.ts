...
import { parseActualSetsJson } from '../network/json-codecs'
...
export async function fetchTodaySession(userId: string, today: string): Promise<SessionRow | null> {
...
export async function fetchCompletedSessions(
  ...
  return (data ?? []).map((row) => {
    ...
    const log = logs[0] as { cycle_phase?: string | null; session_rpe?: number | null } | undefined
    return {
      id: row.id,
      primary_lift: parseLift(row.primary_lift),
      intensity_type: parseIntensity(row.intensity_type),
...
export async function fetchProgramSessionStatuses(
  ...
  if (error) throw error
  return (data ?? []).map((row) => ({ status: parseSessionStatus(row.status) }))
}
...
export async function fetchRecentLogsForLift(
  ...
  if (error) throw error
  return (data ?? []).map((row) => {
    const session = normalizeJoinedSession(row.sessions as SessionJoinLiftIntensity | SessionJoinLiftIntensity[] | null)
    return {
      session_id: row.id,
...
export async function fetchCurrentWeekLogs(
  ...
  if (error) throw error
  return (data ?? []).map((row) => {
    const session = normalizeJoinedSession(row.sessions as SessionJoinLift | SessionJoinLift[] | null)
    return {
