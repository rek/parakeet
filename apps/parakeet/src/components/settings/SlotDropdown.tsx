...
export function SlotDropdown({ label, value, pool, onChange }: SlotDropdownProps) {
  ...
  const styles = useMemo(() => StyleSheet.create({
    ...
    triggerText: { flex: 1, fontSize: 14, color: colors.text, fontWeight: '500' },
            ...
            <FlatList
              ...
              renderItem={({ item: ex }) => (
                <TouchableOpacity style={styles.option} onPress={() => select(ex)} activeOpacity={0.7}>
                  <Text style={[styles.optionText, ex === value && styles.optionSelected]}>
                    {ex}
                  </Text>
