...
import { getPerformanceTrends, getWeeklySetsPerLift } from '../../lib/performance'
...
function buildVolumeChartData(weeklyData: WeeklyVolRow[]) {
  ...
  const datasets = (['squat', 'bench', 'deadlift'] as Lift[]).map(lift => {
    ...
    return {
      data: weeks.map(week => {
        const entry = weeklyData.find(d => d.weekStart === week && d.lift === lift)
        return entry?.setsCompleted ?? 0
      }),
      color: (opacity = 1) => hex + Math.round(opacity * 255).toString(16).padStart(2, '0'),
      strokeWidth: 2,
    }
  })
...
function TrendCard({ trend }: { trend: PerformanceTrend }) {
  ...
  return (
    <TouchableOpacity
      ...
      activeOpacity={0.75}
    >
      <Text style={styles.trendLiftName}>{LIFT_LABELS[trend.lift] ?? trend.lift}</Text>
...
function SessionRow({ session }: { session: CompletedSession }) {
  ...
  const liftName = LIFT_LABELS[session.primary_lift as Lift] ?? session.primary_lift
  const intensityName = intensityLabel[session.intensity_type] ?? session.intensity_type

  return (
    <View style={styles.sessionRow}>
      <View style={styles.sessionRowLeft}>
        <Text style={styles.sessionRowTitle}>{liftName} — {intensityName}</Text>
...
export default function HistoryScreen() {
  ...
  const filteredSessions = liftFilter === 'all'
    ? (sessionsQuery.data ?? [])
    : (sessionsQuery.data ?? []).filter(s => s.primary_lift === liftFilter)

  const volumeChartData = volumeQuery.data ? buildVolumeChartData(volumeQuery.data) : null
      ...
      >
        ...
        {volumeChartData ? (
          <View style={styles.chartCard}>
            <LineChart
              ...
              chartConfig={{
                ...
                color: (opacity = 1) => `${palette.lime400}${Math.round(opacity * 255).toString(16).padStart(2, '0')}`,
            ...
            <View style={styles.legendRow}>
              {(['squat', 'bench', 'deadlift'] as Lift[]).map(lift => (
                <View key={lift} style={styles.legendItem}>
                  <View style={[styles.legendDot, { backgroundColor: LIFT_COLORS[lift] }]} />
          ...
          <Text style={[styles.emptyText, { marginBottom: spacing[8] }]}>
            {volumeQuery.isLoading ? 'Loading…' : 'Complete 2+ weeks of sessions to see volume trends.'}
          </Text>
        )}
        ...
        <View style={styles.filterRow}>
          {(['all', 'squat', 'bench', 'deadlift'] as LiftFilter[]).map(f => (
            <TouchableOpacity
              key={f}
              style={[styles.filterChip, liftFilter === f && styles.filterChipActive]}
              onPress={() => setLiftFilter(f)}
              activeOpacity={0.7}
            >
              <Text style={[styles.filterChipText, liftFilter === f && styles.filterChipTextActive]}>
...
const styles = StyleSheet.create({
  ...
  trendMeta: { fontSize: typography.sizes.xs, color: colors.textTertiary, lineHeight: 16 },
  ...
  sessionRowDate:  { fontSize: typography.sizes.sm, color: colors.textSecondary },
  sessionRowMeta: {
    flexDirection: 'row',
    alignItems: 'center',
    gap: spacing[2],
  ...
  phaseTag: { borderRadius: radii.xs, paddingHorizontal: spacing[1.5], paddingVertical: 2 },
  phaseTagText: { fontSize: typography.sizes.xs, fontWeight: typography.weights.semibold },
  completedBadge: {
  ...
  emptyText: { fontSize: typography.sizes.base, color: colors.textTertiary, marginBottom: spacing[8] },
  // Programs
  programRow: {
    flexDirection: 'row',
    alignItems: 'center',
