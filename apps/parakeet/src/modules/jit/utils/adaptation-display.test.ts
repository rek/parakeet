...
describe('computeDisplayWeights', () => {
  it('uses planned weights when there is no adaptation', () => {
    const result = computeDisplayWeights(
      [makeActual({ weight_grams: 80000 }), makeActual({ weight_grams: 80000 })],
  ...
  it('applies adapted weights for uncompleted sets on weight_reduced adaptation', () => {
    const adaptation = { adaptationType: 'weight_reduced', sets: [{ weight_kg: 85 }, { weight_kg: 85 }] }
    const result = computeDisplayWeights(
      [makeActual({ is_completed: false }), makeActual({ is_completed: false })],
  ...
  it('applies adapted weights for uncompleted sets on sets_capped adaptation', () => {
    const adaptation = { adaptationType: 'sets_capped', sets: [{ weight_kg: 90 }] }
    const result = computeDisplayWeights(
  ...
  it('does not apply adaptation to completed sets', () => {
    const adaptation = { adaptationType: 'weight_reduced', sets: [{ weight_kg: 75 }] }
    const result = computeDisplayWeights(
  ...
  it('does not override weight for reps_reduced adaptation type', () => {
    const adaptation = { adaptationType: 'reps_reduced', sets: [{ weight_kg: 70 }] }
    const result = computeDisplayWeights(
      [makeActual({ is_completed: false })],
      [makePlanned(100)],
      adaptation,
