...
export async function reportDisruption(
    ...
    affectedSessions =
      input.affected_lifts && input.affected_lifts.length > 0
        ? rows.filter((s) => {
          ...
          return parsed.success && input.affected_lifts!.includes(parsed.data)
        })
        : rows
  ...
  const parsedDisruption: TrainingDisruption = DisruptionSchema.parse(disruption)
  const suggested_adjustments = parseAdjustmentSuggestionsJson(
    suggestDisruptionAdjustment(parsedDisruption, toSuggestedSessions(affectedSessions)),
  )
  ...
  if (sessionIds.length > 0) {
    ...
    affectedSessions =
      disruption.affected_lifts && disruption.affected_lifts.length > 0
        ? rows.filter((s) => {
          const parsed = LiftSchema.safeParse(s.primary_lift)
          return parsed.success && disruption.affected_lifts!.includes(parsed.data)
        })
        : rows
  }
