...
export async function reorderAuxiliaryPool(
  ...
  await typedSupabase.from('auxiliary_exercises').delete().eq('user_id', userId).eq('lift', lift)
...
export async function getActiveAssignments(
  ...
  const { data, error } = await typedSupabase
    ...
    .select('lift, exercise_1, exercise_1_locked, exercise_2, exercise_2_locked')
    .eq('user_id', userId)
...
export async function getAllBlockAssignments(
  ...
  const { data, error } = await typedSupabase
    .from('auxiliary_assignments')
    .select('block_number, lift, exercise_1, exercise_1_locked, exercise_2, exercise_2_locked')
    .eq('user_id', userId)
    .eq('program_id', programId)

  if (error) throw error

  const result: Partial<Record<1 | 2 | 3, Partial<Record<Lift, SlotAssignment>>>> = {}
