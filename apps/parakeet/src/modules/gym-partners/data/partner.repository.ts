...
export async function createInvite() {
  ...
  // Token and expires_at are both generated by Postgres (column DEFAULTs).
  // crypto.randomUUID() is not available in React Native / Hermes; client clock
  // must not set expires_at — use server now() so clock skew can't produce a
  // born-expired invite.
  const { data, error } = await typedSupabase
